...
- The handshake costs about 16 times more: **0.88 ms to 14.6 ms** on the same
  machine, and `createIdentity` 0.26 ms to 10.7 ms because it now signs its own
  certificate once. The invite token goes 1687 to 8707 characters and the accept
  3186 to 10206. **The message envelope does not move at all**, still 34 bytes
  of overhead, so authentication is a per conversation cost and not a per
  message one.
- **The invite carries an identity certificate rather than a fresh signature.**
  The first cut of this release signed the conversation id together with the
  sender identity on every invite, at 8 ms each. That is the most expensive
  operation in the library, repeated forever, to re-state a fact that never
  changes, and it took the handshake to 25.8 ms. The certificate is signed once
  at identity creation, so `invite` is now 0.01 ms.

  It is not a weakening. Both shapes are signed by the identity's own signing
  key, so neither ever proved possession of the X25519 or ML-KEM halves; the
  fingerprint and the accept transcript are what catch a substituted identity,
  and both are unchanged. The conversation id inside the old signature bound an
  invite to one conversation while invites stayed replayable verbatim anyway, so
  it was never freshness. A test asserts that re-stamping an invite onto another
  conversation is accepted, rather than leaving that implied.
- **Source maps are no longer published.** They were 766 kB of a 1.6 MB install
  and are useful only to somebody debugging this repository. The tarball goes
  from 456 kB to 262 kB and the unpacked tree from 1.6 MB to 844 kB. They are
  still generated by `npm run build` for local work.
- `dist` is not minified and will not be. It is a crypto library: somebody
  should be able to read the code they installed.
